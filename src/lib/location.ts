 ...
 */
export async function geocodeAddress(address: string): Promise<CoordsPoint | null> {
    try {
 ...
 */
export async function reverseGeocodeCoords(point: CoordsPoint): Promise<string> {
    try {
        const [result] = await Location.reverseGeocodeAsync(point);
        if (result) {
            return `${result.street || result.name || ''}, ${result.subregion || result.region || result.city || ''}`;
        }
    } catch (error) {
